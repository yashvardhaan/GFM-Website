...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('M 947.518')) {
     continue; // skip the broken paths entirely
  }
  newLines.push(lines[i]);
}

code = newLines.join('\n');
// Now we need to insert the original swoosh correctly.
// It goes right BEFORE 'M 590 153.500' 
code = code.replace("M 590 153", orig + "M 590 153");
// and make sure there's no duplicates
const count = (code.match(/M 590 153.500/g) || []).length;
if (count > 0) {
  // Good
}
