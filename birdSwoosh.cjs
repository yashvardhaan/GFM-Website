...
function bend(x, y) {
    ...
    // They want 38% like a bird taking a lift.
    // Let's create an upward swoop using a quadratic shift:
    // delta Y will be proportional to dx^2.
    // This gives it that "lift" curvature.
    const bendFactor = 0.012; // 0.012 * 60^2 ~ 43 px upwards at the tip
    const sweepUp = bendFactor * dx * dx;
    
    // A slight compression in X to maintain path "length" feeling
    // theta ~ atan2(dy, dx) = atan2(bendFactor * 2 * dx, 1)
    const angle = Math.atan2(bendFactor * 2 * dx, 1);
    const xCompress = dx * (1 - Math.cos(angle)) * 0.4; // soften it
...
console.log("Arrowhead bent upwards like a bird taking a lift!");
